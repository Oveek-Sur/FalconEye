...
version = "8.6 (Gobuster Command Stabilization)"
...
def run_gobuster_live(cmd: List[str], target: str, scan_type: str, timeout: int = 420) -> List[str]:
    ...
    try:
        ...
        # 💡 CRITICAL FIX: Manually ensure the line is cleared after the process terminates
        ...
        # 💡 CRITICAL FIX: Line clear even if an exception occurs
        sys.stdout.write('\r' + ' ' * 80 + '\r')
        sys.stdout.flush()
...
# Helper function to check for non-blocking input (removed termios/tty dependency)
def run_nuclei_scan(live_hosts: List[str], scan_timeout: int) -> List[str]:
    """Runs nuclei for a quick scan against live hosts with progress simulation (No interactive quit)."""
    results = []
    ...
    print_colored(f"\n  [>] Starting nuclei scan (Templates: {templates}, Max time: {scan_timeout}s).", Colors.CYAN)
    ...
    with tempfile.NamedTemporaryFile(mode='w+', delete=True) as temp_input:
        ...
        try:
            # We must use Popen to show progress but since no interactive quit, we rely on timeout/completion
            ...
            while process.poll() is None:
                ...
                time.sleep(0.5) # Update frequently
    ...
    if shutil.which("gobuster"):
        ...
        if wordlist_path:
            ...
            cmd = [
                ...
                "-w", wordlist_path,
                # 💡 FIX: Adjusted to 100 threads as requested
                "-t", "100", 
                "--timeout", "10s", # Reduced individual timeout
                "-r", "8.8.8.8", # Added fast DNS resolver for performance
                "--wildcard" # Added wildcard flag
            ]
            # Gobuster DNS runs live and handles its own output
            # 💡 FIX: Timeout is 5 minutes (300s) as requested
